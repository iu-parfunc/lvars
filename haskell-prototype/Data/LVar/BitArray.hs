...
-- forVec :: M.IOVector a -> (Int -> a -> IO ()) -> IO ()
forVec :: Storable a =>
...
put !ix !elm (BitArray (WrapLVar lv)) = WrapPar$ putLV lv (putter ix)
...
waitElem :: Ord a => a -> BitArray s a -> Par d s ()
waitElem !elm (BitArray (WrapLVar lv)) = WrapPar $
    getLV lv globalThresh deltaThresh
  where
