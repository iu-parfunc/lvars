...
freezeAfterCallbacks :: ISet a -> (a -> Par ()) -> Par () -> Par (S.Set a)
freezeAfterCallbacks lv callback action =
    do freezeLVAfter lv initCB (\x -> return$ Just$ callback x)
  ...
  where 
    initCB ref = do
      ...
      return $ Just $ do 
        ...
        action -- Any additional puts here trigger the callback.
