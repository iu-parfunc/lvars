...
parMap :: (Traversable t, NFData b, ParFuture p, HasPut e, HasGet e) =>
          (a -> b) -> t a -> p e s (t b)
{-# INLINE parMap #-}
parMap f xs = mapM (spawnP . f) xs >>= mapM read

-- | A variant of `ParMap` that only evaluates to weak-head-normal-form.
parMap_ :: (Traversable t, ParFuture p, HasGet e, HasPut e) =>
          (a -> b) -> t a -> p e s (t b)
{-# INLINE parMap_ #-}
parMap_ f xs = mapM spawnWHNF xs >>= mapM read
  where
...
--
ptraverse :: (Traversable t, NFData b, ParFuture p, HasPut e, HasGet e) =>
           (a -> p e s b) -> t a -> p e s (t b)
{-# INLINE ptraverse #-}
ptraverse f xs = mapM (spawn . f) xs >>= mapM read


-- | A variant that only evaluates to weak-head-normal-form.
ptraverse_ :: (Traversable t, ParFuture p, HasPut e, HasGet e) =>
           (a -> p e s b) -> t a -> p e s (t b)
{-# INLINE ptraverse_ #-}
ptraverse_ f xs = mapM (spawn_ . f) xs >>= mapM read
